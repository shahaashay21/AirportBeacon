		...
		if(err){
			...
			if(!results){
				...
				stream.on('end',function(){
					...
					fs.readFile(__dirname + '/user.png', function(err, squid){
						if (err) { console.log(err) } ;
						img.src = squid;
												
						

						var j =req.param("beacon");
						//j=6;						
						if(j<4){ y=30 } else { y=185 }					
						if(j%3==1){x=30} else if(j%3==2){x=190} else {x=340}	
					
						ctx1.drawImage(img, x, y);
						ctx1.save();
						var out1 = fs.createWriteStream(__dirname + '/../public/user.png')
							  , stream1 = canvas.createPNGStream();
						...
						for(i=1;i<7;i++){
							...
							if(i%3==1){x=70} else if(i%3==2){x=220} else {x=370}	
							//console.log(i+"x :: "+x+" y::"+y);
							ctx1.fillStyle = "#00ff00";  //<======= and here
						...
						stream1.on('end',function(chunk){
							console.log("user png saved");
						});				
					});				
					//console.log(img);								
				});				
